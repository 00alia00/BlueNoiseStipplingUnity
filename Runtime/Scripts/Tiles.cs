...
namespace BlueNoise
{
    ...
    public class WangTile
    {
        ...
        public delegate void WangTileDebugStepDelegate(PointSet img, string description);
                                         ...
                                         out List<PoissonSample> result)
        {
            ...
            PointSet debugImage = null;
            if (OnDebugStep != null) debugImage = new PointSet(800, 800);
    ...
    public class WangTileSet
    {
        ...
        public static WangTileSet FromFile(string sourceFile)
        {
            ...
            System.IO.StreamReader file = new System.IO.StreamReader(sourceFile + ".xml");
        ...
        /// </summary>
        public void ToColorTiles(PointSet img, Size tileSize)
        {
            //Color[] colors = new Color[numColors];
            //Random r = new Random();
            //for (int i = 0; i < numColors; i++) colors[i] = Color.FromArgb(r.Next(256), r.Next(256), r.Next(256));

            List<PointSet> tileImgs = new List<PointSet>();
            foreach (WangTile wt in tiles)
            {
                PointSet bmp = new PointSet(tileSize.Width, tileSize.Height);
        ...
        public void ToNoiseTiles(PointSet img, Size tileSize, float coverage)
        {
            List<PointSet> tileImgs = new List<PointSet>();
            foreach (WangTile wt in tiles)
            {
        ...
        public void Serialize(string destFile)
        {
            ...
            System.IO.StreamWriter file = new System.IO.StreamWriter(destFile + ".xml");
            serializer.Serialize(file, this);
            file.Close();
        }

        public void SerializeBinary(string destFile)
        {
            using (System.IO.BinaryWriter writer = new System.IO.BinaryWriter(System.IO.File.Open(destFile + ".bin", System.IO.FileMode.Create)))
            {
                ...
                foreach (var tile in tiles)
                {
                    ...
                    {
                        writer.Write(tile.edgeColors[colour]);
                    }

                    writer.Write(tile.distribution.Count);

                    for (int dist_idx = 0; dist_idx < tile.distribution.Count; dist_idx++)
                    {
                        writer.Write(tile.distribution[dist_idx].x);
                        writer.Write(tile.distribution[dist_idx].y);
                        writer.Write(tile.distribution[dist_idx].radius);
                        writer.Write(tile.distribution[dist_idx].ranking);
                    }
                }
            }
        }

        public static WangTileSet FromBinaryFile(string sourceFile)
        {
            WangTileSet wts = new WangTileSet();
            if (System.IO.File.Exists(sourceFile + ".bin"))
            {
                using (System.IO.BinaryReader reader = new System.IO.BinaryReader(System.IO.File.Open(sourceFile + ".bin", System.IO.FileMode.Open)))
                {
                    wts.numColors = reader.ReadInt32();
                    int num_tiles = reader.ReadInt32();
                    wts.tiles = new List<WangTile>();

                    for (int tile_idx =0;tile_idx< num_tiles;tile_idx++)
                    {
                        WangTile tile = new WangTile();
                        tile.edgeColors = new int[4];

                        for (int colour = 0; colour < 4; colour++)
                        {
                            tile.edgeColors[colour] = reader.ReadInt32();
                        }

                        int num_distributions = reader.ReadInt32();
                        tile.distribution = new List<PoissonSample>();
                        for (int dist_idx = 0; dist_idx < num_distributions; dist_idx++)
                        {
                            tile.distribution.Add( new PoissonSample() {
                                x = reader.ReadSingle(),
                                y = reader.ReadSingle(),
                                radius = reader.ReadSingle(),
                                ranking = reader.ReadSingle()
                            });
                        }
        ...
        private void ToImage(PointSet img, Size tileSize, List<PointSet> tileImgs)
        {
